...
em=item.end_time.minute 
   ...
   def handle_in("nilam_setting", payload, socket) do
               ...
               project_nilam =     Repo.all(from s in School.Affairs.ProjectNilam,
                where: s.standard_id ==^payload["level"],select: %{ids: s.id,below_satisfy: s.below_satisfy,
                count_page: s.count_page,import_from_library: s.import_from_library,member_reading_quantity: s.member_reading_quantity,
                page: s.page,standard_id: s.standard_id})|>hd
 ...
 standard_level=payload["standard_level"]
               standard_subject = Repo.all(from s in School.Affairs.StandardSubject,
                ...
                semester_id: s.semester_id,standard_id: s.standard_id,subject_id: r.description})
 ...
 standard_level=payload["standard_level"]
               subject_test = Repo.all(from s in School.Affairs.ExamMaster,
                ...
                left_join: r in School.Affairs.Subject, on: r.id==p.subject_id,
                where: s.level_id ==^payload["standard_level"],select: %{year: s.year,
                semester_id: s.semester_id,standard_id: s.level_id,subject_id: r.description,name: s.name})
 ...
 standard_level=payload["standard_level"]
               all = Repo.all(from s in School.Affairs.Period,
                left_join: p in School.Affairs.Class, on: s.class_id== p.id,
                 left_join: r in School.Affairs.Subject, on: r.id==s.subject_id,
                  left_join: e in School.Affairs.Teacher, on: e.id==s.teacher_id,
                where: p.level_id ==^payload["standard_level"],select: %{day: s.day,
                start_time: s.start_time,end_time: s.end_time,subject_name: r.description,teacher_name: e.name,class_name: p.name})


               period=for item <- all do
                ...
                end_time=item.start_time|>Time.to_string()|>String.split_at(5)|>elem(0)
                 ...
                 %{day: item.day,
                start_time: start_time,end_time: end_time,subject_name: item.subject_name,teacher_name: item.teacher_name,class_name: item.class_name}
               end
 ...
 class_id=payload["class_id"]
  ...
  broadcast(socket, "show_class_info", %{id: all.id,name: all.name,remark: all.remarks,institution_id: all.institution_id,level_id: all.level_id})
    {:noreply, socket}
 ...
 class_id=payload["class_id"]
  ...
  class = Repo.get_by(School.Affairs.Class, %{id: class_id})
               all = Repo.all(from s in School.Affairs.SubjectTeachClass,
                left_join: g in School.Affairs.Subject,on: g.id==s.subject_id,
                left_join: r in School.Affairs.Teacher,on: r.id==s.teacher_id,
  ...
  broadcast(socket, "show_class_subject", %{class_id: class_id,all: all})
   ...
   period=Repo.all(from p in School.Affairs.Period,
    left_join: r in School.Affairs.SubjectTeachClass, on: r.subject_id==p.subject_id,
    left_join: s in School.Affairs.Subject, on: s.id==r.subject_id,
    left_join: t in School.Affairs.Teacher, on: t.id==r.teacher_id,
    left_join: d in School.Affairs.Day, on: d.name==p.day,
...
em=item.end_time.minute 
      ...
      students_in =
        Repo.all(
          from(
            ...
            on: t.id == s.sudent_id,
            where:
              s.institute_id == ^payload["inst_id"] and
                s.semester_id == ^payload["semester_id"] and
  ...
  def handle_in("class_student_info",payload,socket) do
     ...
     students =
      Repo.all(
        from(
          s in Student,
          left_join: g in StudentClass, on: s.id==g.sudent_id,
 ...
 class_id=payload["class_id"]
      ...
      class = Repo.get_by(School.Affairs.Class, %{id: class_id})
       ...
       standard_subject = Repo.all(from s in School.Affairs.StandardSubject,
         ...
         select: %{ subject_name: g.description,
                  subject_code: g.code,
                  subject_id: g.id,
                  id: s.id })


         teacher=Repo.all( from r in School.Affairs.Teacher,select: %{ teacher_name: r.name,
                  teacher_code: r.code,id: r.id})


         data= Repo.all(from s in School.Affairs.SubjectTeachClass,
                left_join: g in School.Affairs.Subject,on: g.id==s.subject_id,
                left_join: r in School.Affairs.Teacher,on: r.id==s.teacher_id,
                where: s.class_id==^class_id and s.standard_id==^class.level_id,
                select: %{
 ...
 class_id=payload["class_id"]
     ...
     all =
      Repo.all(
        from(
          ...
          on: t.id == p.teacher_id,where: p.class_id==^class_id,
    ...
    exam =
      Repo.all(
        from(
          e in School.Affairs.ExamMaster,where: e.level_id==^class.level_id,
...
class_id = payload["class_id"]
 ...
 news = mark1 |> Enum.group_by(fn x -> x.student_name end)
      ...
      Phoenix.View.render_to_string(
        SchoolWeb.ExamView,
        "rank.html",z: k, exam_name: exam_name, mark: mark, mark1: mark1)
   ...
   def handle_in("exam_result_standard",payload,socket) do
     ...
     exam = Repo.all(from(e in School.Affairs.ExamMaster,where: e.level_id==^payload["standard_id"]))|>Enum.map(fn x -> %{id: x.id,exam_name: x.name} end)|>Enum.uniq


              html =
      Phoenix.View.render_to_string(
        SchoolWeb.ExamView,
        "exam_standard_filter.html",exam: exam)
    ...
    all=for item <- exam_standard do
      ...
      Phoenix.View.render_to_string(
        SchoolWeb.ExamView,
        "exam_ranking.html", z: t, exam_name: exam_name, mark: mark, mark1: mark1)
    ...
    {:noreply, socket}
       ...
       broadcast(socket, "show_exam_record_standard_error", %{action: "Please Insert Exam Record First"})
    {:noreply, socket}
